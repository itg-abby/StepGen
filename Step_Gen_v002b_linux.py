...
from aubio import source, onset, pitch, freqtomidi

import subprocess

# AbderRahman (Abby) 's SM file generator v0.2
#
...
# Prepare AUBIO data of soundfile
	...
	if len(sys.argv) > 3 and ('-p' in sys.argv):
		...
		pitchType = "{0}".format(sys.argv[ptindex])
	else:
		pitchType = 'default'
...
#	aubiopit = aubiopitch(sent_file, pitchType, ptolerance, sr)
	aubioNOTES(sent_file, onsetType, thresholdVal, silenceVal, pitchType, ptolerance)
	aubiodat = []
...
#	silence = -10
	...
	with open('audata_onset','w') as datas:
		for item in onsets:
			datas.write('\n{0}'.format(item))
	return onsets
...
# AUBIO datagen
# - p pitchmethod (seelist) -l pitch tolerance (0.2-0.9)
...
# AUBIO datagen
def aubioNOTES(filename, onsetType, threshold, silence, pitchType, ptolerance):
#	runstring = 'aubionotes -i {0} -v'.format(filename)
	param_list = ' '
	if (onsetType != ''):
		param_list += '-O {0} '.format(onsetType)
	if (silence != ''):
		param_list += '-s {0} '.format(silence)
	if (threshold != ''):
		param_list += '-t {0} '.format(threshold)
	if (ptolerance != ''):
		param_list += '-l {0} '.format(ptolerance)
	if (pitchType != ''):
		param_list += '-p {0} '.format(pitchType)

	raN = subprocess.Popen(['./aubionotes_local -i {0} {1} -v'.format(filename, param_list)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
